...
| `jira_list_saved_filters` | — | The saved filters this account has favourited in Jira: the id, the name, the description, the JQL each runs, and who owns it. Discovery only — a filter is run by naming `filter = <id>` in `jira_search`, which is ordinary JQL. Jira Server publishes no endpoint for every filter an account can see, so a saved filter nobody favourited is not listed. |
...
| `jira_get_issues` | — | Up to 20 issues in one call, with `include` opting into `comments`, `transitions`, `changelog`, `links`, `worklogs`, `attachments` and `subtasks` for each. Each key succeeds or fails on its own, and the transition list arrives with the issue the agent is about to transition. |
...
`TRANSITION_ISSUES`, bare, as the permission-scheme screen spells them — and says whether the
account holds it, asked of Jira after the refusal and never before. Where the account does hold it,
the message says so and names any other write permission it lacks in the same place, because a
`403` also comes from an instance in read-only mode and from a throttled login. This server still
models nothing about Jira's permission scheme and still does not retry a `403`; the answer is not a
promise that the next write will succeed (ADR-0013). Check which account it is with `jira_whoami`,
